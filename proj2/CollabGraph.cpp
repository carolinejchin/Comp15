 ...
 **   Dec 2021: cchin01
 **       implemented populate_graph, get_vertex_neighbors, and report_path
 **/
    ...
    for (int i = 0; i < artists.size(); i++) {
        for (int j = 0; j < artists.size(); j++) {
            if (i != j and artists[i].get_collaboration(artists[j]) != "") {
                insert_edge(artists[i], artists[j], artists[i].get_collaboration(artists[j]));
            }
        }
    }
}
...
vector<Artist> CollabGraph::get_vertex_neighbors(const Artist& artist) const {
    vector<Artist> a;
    Vertex *v;
    v = graph.at(artist.get_name());
    for (int i = 0; i < v->neighbors.size(); i++) {
        a.push_back(v->neighbors[i].neighbor->artist);
    }
    return a;
}
